...
def unfollow(username):
    ...
    if form.validate_on_submit():
        user = User.query.filter_by(username=username).first()
        if user is None:
            flash(_('User %s(username) not found', username=username))
            return redirect(url_for('index'))
        if user == current_user:
        ...
        db.session.commit()
        flash(_('You are not following %s(username)', username=username))
        return redirect(url_for('user', username=username))
    else:
        return redirect(url_for('index'))
...
def explore():
    ...
    next_url = url_for('explore', page=posts.next_num) if \
        posts.has_next else None
    prev_url = url_for('explore', page=posts.prev_num) if \
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash(_("Check your email for the instructions to reset password"))
        return redirect(url_for('login'))
    return render_template(
        'reset_password_request.html', title='Reset Password',
        form=form
    )
